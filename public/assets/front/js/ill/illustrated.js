(function ($) {
  ...
  function openQuickInline(id, url, sku, user) {
    const base  = window.ILL_ROUTES?.quick || '/modal/quickview/';
    const title = t('catalog.quickview.title');
    let finalUrl = (url && typeof url === 'string') ? url : (id ? (base + id) : null);
    if (finalUrl && user && finalUrl.indexOf('user=') === -1) {
      finalUrl += (finalUrl.indexOf('?') === -1 ? '?' : '&') + 'user=' + encodeURIComponent(user);
...
//       // هنا تستمر باستخدام id/sku/url كما في منطقك الأصلي
//       if (!$inAlt && sku) {
//         openAlternativeInline(sku);
//         return;
//       }
...
//     const id  = $(btn).data('id');
//     if (!id) {
//       console.warn('ill-add-to-cart: missing data-id');
//       return;
//     }
...
//       const id  = $(btn).data('id');
//       if (!id) {
//         console.warn('ill-buy-now: missing data-id');
//         return;
//       }
