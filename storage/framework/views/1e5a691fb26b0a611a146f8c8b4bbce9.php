...
(function() {
    ...
    function searchVin() {
        ...
        if (query.length < 10) {
            showError('<?php echo e(__("VIN must be at least 10 characters.")); ?>');
            return;
        }
        ...
        // إرسال الطلب
        fetch('<?php echo e(route("api.search.vin")); ?>?query=' + encodeURIComponent(query), {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            ...
            if (data.success && data.result) {
                ...
                showError(data.message || '<?php echo e(__("VIN not found.")); ?>');
            }
        })
        .catch(error => {
            setLoading(false);
    ...
    function selectVin() {
        ...
        fetch('<?php echo e(route("api.search.vin.select")); ?>', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success && data.redirect_url) {
                ...
                setLoading(false);
                showError(data.message || '<?php echo e(__("Failed to select VIN.")); ?>');
            }
        })
        .catch(error => {
            setLoading(false);
...
<?php /**PATH C:\Users\hp\Herd\new\resources\views/includes/frontend/search-vin-ajax.blade.php ENDPATH**/ ?>