...
class DiscountCodeController extends VendorBaseController
{
    //*** JSON Request
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('status', function (DiscountCode $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('vendor-discount-code-status', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>' . __("Activated") . '</option><option data-val="0" value="' . route('vendor-discount-code-status', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . __("Deactivated") . '</option></select></div>';
            })
            ->addColumn('action', function (DiscountCode $data) {
                return '<div class="action-list"><a href="' . route('vendor-discount-code-edit', $data->id) . '"> <i class="fas fa-edit"></i>' . __('Edit') . '</a><a href="javascript:;" data-href="' . route('vendor-discount-code-delete', $data->id) . '" data-bs-toggle="modal" data-bs-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
            })
    ...
    public function index()
    {
        ...
        return view('vendor.discount-code.index', compact('datas'));
    }

    //*** GET Request
    public function create()
    {
        return view('vendor.discount-code.create');
    }
    ...
    public function edit($id)
    {
        ...
        return view('vendor.discount-code.edit', compact('data'));
    }
