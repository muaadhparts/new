...
class MerchantMembershipPlanController extends OperatorBaseController
{
    //*** GET Request
                            ...
                            ->addColumn('action', function(UserMembershipPlan $data) {
                                ...
                                if($data->status == 0){
                                    ...
                                    $status =  '<select class="process select merchant-droplinks '.$class.'">
                                                    <option data-val="1" value="'. route('operator-user-membership-plan-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>
                                                    '.__("Completed").
                                                    '</option>
                                                    <option data-val="0" value="'. route('operator-user-membership-plan-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>
                                                    '.__("Pending").'
                                                    </option>
                                                </select>';
                                }

                                return '<div class="action-list">'.$status.'<a data-href="' . route('operator-merchant-membership-plan',$data->id) . '" class="view details-width" data-bs-toggle="modal" data-bs-target="#modal1"> <i class="fas fa-eye"></i>'.__('Details').'</a></div>';
                            })
                            ->rawColumns(['action'])
    ...
    {
        if($slug == 'completed'){
            return view('operator.merchant.membership-plans');
        }else if($slug == 'pending'){
            return view('operator.merchant.pending-membership-plans');
        }
    ...
    public function show($id)
    {
        ...
        return view('operator.merchant.membership-plan-details',compact('membershipPlan'));
    }
