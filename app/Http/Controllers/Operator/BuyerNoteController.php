...
class BuyerNoteController extends OperatorBaseController
{
	...
	{
		$datas = BuyerNote::with(['catalogItem', 'merchantItem.user', 'merchantItem.qualityBrand', 'merchantItem.brand', 'user'])
			->latest('id')
			->get();
	...
	{
		// brand is now on merchant_items (2026-01-20)
		$data = BuyerNote::with(['catalogItem', 'merchantItem.user', 'merchantItem.qualityBrand', 'merchantItem.brand'])->findOrFail($id);
