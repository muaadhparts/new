...
class ImportController extends OperatorBaseController
{
    //*** JSON Request
            ...
            ->addColumn('status', function (MerchantItem $mi) {
                ...
                return '<div class="action-list">
                    <select class="process select droplinks ' . $class . '">
                        <option data-val="1" value="' . route('operator-merchant-catalogItem-status', ['id' => $mi->id, 'status' => 1]) . '" ' . $s . '>' . __("Activated") . '</option>
                        <option data-val="0" value="' . route('operator-merchant-catalogItem-status', ['id' => $mi->id, 'status' => 0]) . '" ' . $ns . '>' . __("Deactivated") . '</option>
                    </select>
                </div>';
            })
            ->addColumn('action', function (MerchantItem $mi) {
                ...
                return '<div class="godropdown"><button class="go-dropdown-toggle"> ' . __("Actions") . '<i class="fas fa-chevron-down"></i></button>
                    <div class="action-list">
                        <a href="' . route('operator-import-edit', $catalogItem->id) . '"><i class="fas fa-edit"></i> ' . __("Edit") . '</a>
                        ...
                        <a data-href="' . route('operator-prod-feature', $catalogItem->id) . '" class="feature" data-bs-toggle="modal" data-bs-target="#modal2"> <i class="fas fa-star"></i> ' . __("Highlight") . '</a>
                        <a href="javascript:;" data-href="' . route('operator-affiliate-prod-delete', $catalogItem->id) . '" data-bs-toggle="modal" data-bs-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> ' . __("Delete") . '</a>
                    </div></div>';
            })
    ...
    public function index(){
        return view('operator.catalog-item-import.index');
    }
    ...
    public function createImport()
    {
        ...
        $sign = $this->curr;
        return view('operator.catalog-item-import.createone',compact('cats','sign'));
    }
    ...
    public function importCSV()
    {
        $cats = collect(); // Category system removed - using TreeCategories
        $sign = $this->curr;
        return view('operator.catalog-item-import.importcsv',compact('cats','sign'));
    }
    ...
    public function store(Request $request)
    {
        ...
        //--- Redirect Section
        $msg = __("New Affiliate CatalogItem Added Successfully.").'<a href="'.route('operator-import-index').'">'.__("View CatalogItem Lists.").'</a>';
    ...
    public function edit($id)
    {
        ...
        $data = CatalogItem::findOrFail($id);
        $sign = $this->curr;
        return view('operator.catalog-item-import.editone',compact('cats','data','sign'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        //--- Redirect Section
        $msg = __('CatalogItem Updated Successfully.').'<a href="'.route('operator-import-index').'">'.__("View Catalog Item Lists.").'</a>';
        return response()->json($msg);
        //--- Redirect Section Ends
    }
}
