...
class UserTopUpController extends OperatorBaseController
{
    //*** JSON Request
             ...
             return Datatables::of($datas)
                                ...
                                ->addColumn('action', function(TopUp $data) {
                                    if($data->status == 1){
                                        ...
                                        return '<div class="action-list"><select class="process select merchant-droplinks '.$class.'"><option data-val="1" value="'. route('operator-user-top-up-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>'.__("Completed").'</option><option data-val="0" value="'. route('operator-user-top-up-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.__("Pending").'</option></select></div>';
                                    }
    ...
    {
        if($slug == 'all'){
            return view('operator.top-up.index');
        }else if($slug == 'pending'){
            return view('operator.top-up.pending');
        }
