...
class MerchantSubscriptionController extends AdminBaseController
{
    //*** GET Request
    public function subsdatatables($status)
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('action', function(UserSubscription $data) {
                                ...
                                return '<div class="action-list">'.$status.'<a data-href="' . route('admin-merchant-sub',$data->id) . '" class="view details-width" data-bs-toggle="modal" data-bs-target="#modal1"> <i class="fas fa-eye"></i>'.__('Details').'</a></div>';
                            })
                            ->rawColumns(['action'])
    ...
    {
        if($slug == 'completed'){
            return view('admin.merchant.subscriptions');
        }else if($slug == 'pending'){
            return view('admin.merchant.pending-subscriptions');
        }
    ...
    public function sub($id)
    {
        ...
        return view('admin.merchant.subscription-details',compact('subs'));
    }
