...
class StaticContentController extends AdminBaseController
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ->addColumn('header', function (StaticContent $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-static-content-header', [$data->id, 1]) . '" ' . $s . '>' . __('Showed') . '</option><option data-val="0" value="' . route('admin-static-content-header', [$data->id, 0]) . '" ' . $ns . '>' . __('Not Showed') . '</option></select></div>';
            })
            ->addColumn('footer', function (StaticContent $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-static-content-footer', [$data->id, 1]) . '" ' . $s . '>' . __('Showed') . '</option><option data-val="0" value="' . route('admin-static-content-footer', [$data->id, 0]) . '" ' . $ns . '>' . __('Not Showed') . '</option></select></div>';
            })
            ->addColumn('action', function (StaticContent $data) {
                return '<div class="action-list"><a href="' . route('admin-static-content-edit', $data->id) . '"> <i class="fas fa-edit"></i>' . __('Edit') . '</a><a href="javascript:;" data-href="' . route('admin-static-content-delete', $data->id) . '" data-bs-toggle="modal" data-bs-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
            })
    ...
    {
        return view('admin.static-content.index');
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.static-content.edit', compact('data'));
    }
