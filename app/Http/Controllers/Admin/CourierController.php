...
class CourierController extends AdminBaseController
{
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('action', function (Courier $data) {
                ...
                $ban = '<select class="process select droplinks ' . $class . '">' .
                    '<option data-val="0" value="' . route('admin-courier-ban', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>' . __("Block") . '</option>' .
                    '<option data-val="1" value="' . route('admin-courier-ban', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . __("UnBlock") . '</option></select>';
                ...
                return '<div class="action-list">
                    ...
                    . $ban .
                    '
                    <a href="' . route('admin-courier-show', $data->id) . '" >
                        <i class="fas fa-eye"></i> ' . __("Details") . '
                    </a>

                    <a href="javascript:;" data-href="' . route('admin-courier-delete', $data->id) . '" data-bs-toggle="modal" data-bs-target="#confirm-delete" class="delete">
    ...
    {
        return view('admin.courier.withdraws');
    }
    ...
    public function show($id)
    {
        ...
        return view('admin.courier.show', compact('data'));
    }
    ...
    public function withdrawdatatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('action', function (Withdraw $data) {
                $action = '<div class="action-list"><a data-href="' . route('admin-withdraw-courier-show', $data->id) . '" class="view details-width" data-bs-toggle="modal" data-bs-target="#modal1"> <i class="fas fa-eye"></i> ' . __("Details") . '</a>';
                if ($data->status == "pending") {
                    $action .= '<a data-href="' . route('admin-withdraw-courier-accept', $data->id) . '" data-bs-toggle="modal" data-bs-target="#status-modal1"> <i class="fas fa-check"></i> ' . __("Accept") . '</a><a data-href="' . route('admin-withdraw-courier-reject', $data->id) . '" data-bs-toggle="modal" data-bs-target="#status-modal"> <i class="fas fa-trash-alt"></i> ' . __("Reject") . '</a>';
                }
                $action .= '</div>';
                return $action;
            })
    ...
    public function withdrawdetails($id)
    {
        ...
        return view('admin.courier.withdraw-details', compact('withdraw', 'sign'));
    }
