...
@section('name', ($catalog->name ?? $catalog->shortName ?? $catalog->code) . ' - ' . __('Categories'))
...
<section class="gs-breadcrumb-section mb-4">
    <div class="container">
        <div class="row justify-content-center content-wrapper">
            <div class="col-12">
                <h2 class="breadcrumb-name">{{ $catalog->shortName ?? $catalog->name ?? $catalog->code }}</h2>
                <ul class="bread-menu">
                    <li><a href="{{ route('front.index') }}">@lang('Home')</a></li>
                    <li><a href="{{ route('catlogs.index', $brand->name) }}">{{ $brand->localized_name }}</a></li>
                    <li><a href="javascript:;">{{ $catalog->shortName ?? $catalog->name ?? $catalog->code }}</a></li>
                </ul>
            </div>
        </div>
    </div>
