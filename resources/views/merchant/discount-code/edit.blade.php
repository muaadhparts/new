    ...
    <div class="gs-merchant-outlet">
        ...
        <div class="gs-merchant-breadcrumb has-mb">
            <div class="d-flex gap-4 flex-wrap align-items-center">
                <a class="back-btn" href="{{ route('merchant-discount-code-index') }}">
                    <i class="fa-solid fa-arrow-left"></i>
                </a>
                <h4 class="text-capitalize">@lang('Edit Discount Code')</h4>
            </div>
            <ul class="breadcrumb-menu">
                ...
                <li>
                    <a href="{{ route('merchant-discount-code-index') }}" class="text-capitalize">@lang('Discount Codes')</a>
                </li>
                <li>
                    <a href="javascript:;" class="text-capitalize">@lang('Edit')</a>
                </li>
            </ul>
        </div>
        <!-- breadcrumb end -->
        ...
        <div class="merchant-edit-profile-section-wrapper">
            <div class="gs-edit-profile-section">
                <form class="edit-profile-area" id="discountCodeForm" action="{{ route('merchant-discount-code-update', $data->id) }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label for="code">@lang('Discount Code') <span class="text-danger">*</span></label>
                                <input type="text" id="code" class="form-control" placeholder="@lang('Enter Discount Code')"
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label for="apply_to">@lang('Apply to') <span class="text-danger">*</span></label>
                                <select id="apply_to" class="form-control" name="apply_to" required>
                                    <option value="">@lang('Select Type')</option>
                                    <option value="category" {{ $data->apply_to == 'category' ? 'selected' : '' }}>@lang('Category')</option>
                                    <option value="sub_category" {{ $data->apply_to == 'sub_category' ? 'selected' : '' }}>@lang('Sub Category')</option>
                                    <option value="child_category" {{ $data->apply_to == 'child_category' ? 'selected' : '' }}>@lang('Child Category')</option>
                                </select>
                            </div>
                        </div>

                        <div class="col-lg-6 {{ $data->apply_to == 'category' ? '' : 'd-none' }}" id="categoryDiv">
                            <div class="form-group">
                        ...
                        <div class="col-lg-6 {{ $data->apply_to == 'sub_category' ? '' : 'd-none' }}" id="subCategoryDiv">
                            <div class="form-group">
                        ...
                        <div class="col-lg-6 {{ $data->apply_to == 'child_category' ? '' : 'd-none' }}" id="childCategoryDiv">
                            <div class="form-group">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select id="type" class="form-control" name="type" required>
                                    <option value="">@lang('Select Type')</option>
                                    <option value="0" {{ $data->type == 0 ? 'selected' : '' }}>@lang('Percentage Discount')</option>
                                    <option value="1" {{ $data->type == 1 ? 'selected' : '' }}>@lang('Fixed Amount Discount')</option>
                                </select>
                            </div>
                        </div>

                        <div class="col-lg-6 {{ $data->type !== null ? '' : 'd-none' }}" id="priceDiv">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" step="0.01" id="price" class="form-control"
                                        placeholder="@lang('Enter Value')" name="price" value="{{ $data->price }}"
                                        {{ $data->type == 0 ? 'max=100' : '' }}>
                                    <span class="input-group-text" id="priceSuffix">{{ $data->type == 0 ? '%' : ($curr->sign ?? '$') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select id="quantity_type" class="form-control" name="quantity_type" required>
                                    <option value="0" {{ $data->times == null ? 'selected' : '' }}>@lang('Unlimited')</option>
                                    <option value="1" {{ $data->times != null ? 'selected' : '' }}>@lang('Limited')</option>
                                </select>
                            </div>
                        </div>

                        <div class="col-lg-6 {{ $data->times != null ? '' : 'd-none' }}" id="timesDiv">
                            <div class="form-group">
                                <label for="times">@lang('Number of Uses') <span class="text-danger">*</span></label>
                                <input type="number" id="times" class="form-control"
                                    placeholder="@lang('Enter number of uses')" name="times" value="{{ $data->times }}">
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <button class="template-btn btn-forms" type="submit">
                                <i class="fas fa-save"></i> @lang('Update Discount Code')
                            </button>
                        </div>
                    </div>
                </form>
...
<script type="text/javascript">
    ...
    function loadCategories(type, targetSelect, defaultOption, selectedValue) {
        $(targetSelect).html('<option value="">@lang("Loading...")</option>');

        $.ajax({
            url: '{{ route("merchant-discount-code-get-categories") }}',
            type: 'GET',
            data: { type: type },
    ...
    if (initialType === 'category' && initialCategory) {
        loadCategories('category', '#categorySelect', '@lang("Select Category")', initialCategory);
    } else if (initialType === 'sub_category' && initialSubCategory) {
        loadCategories('sub_category', '#subCategorySelect', '@lang("Select Sub Category")', initialSubCategory);
    } else if (initialType === 'child_category' && initialChildCategory) {
        loadCategories('child_category', '#childCategorySelect', '@lang("Select Child Category")', initialChildCategory);
    }

    // Apply To Change - Load categories dynamically
    $('#apply_to').on('change', function() {
        ...
        if (val === 'category') {
            ...
            loadCategories('category', '#categorySelect', '@lang("Select Category")', null);
        } else if (val === 'sub_category') {
            $('#subCategoryDiv').removeClass('d-none');
            loadCategories('sub_category', '#subCategorySelect', '@lang("Select Sub Category")', null);
        } else if (val === 'child_category') {
            $('#childCategoryDiv').removeClass('d-none');
            loadCategories('child_category', '#childCategorySelect', '@lang("Select Child Category")', null);
        }
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (typeof response === 'object' && response.errors) {
                    ...
                    setTimeout(function() {
                        window.location.href = '{{ route("merchant-discount-code-index") }}';
                    }, 1500);
                }
            },
            error: function(xhr) {
