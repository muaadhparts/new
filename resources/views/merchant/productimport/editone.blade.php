...
{{-- $merchantItem is passed from controller --}}
							...
							<div class="body-area">
								...
								<div class="row">
									...
									<div class="col-lg-12">
										<select id="cat" name="category_id" required="">
												...
												@foreach($cats as $cat)
													<option data-href="{{ route('merchant-subcat-load',$cat->id) }}" value="{{$cat->id}}" {{$cat->id == $data->category_id ? "selected":""}} >{{$cat->name}}</option>
												@endforeach
										</select>
									</div>
								</div>
									...
									<div class="col-lg-12">
											<select id="subcat" name="subcategory_id">
												...
												@if($data->subcategory_id == null)
													...
													<option data-href="{{ route('merchant-childcat-load',$sub->id) }}" value="{{$sub->id}}" >{{$sub->name}}</option>
													@endforeach
												@else
													@foreach($data->category->subs as $sub)
													<option data-href="{{ route('merchant-childcat-load',$sub->id) }}" value="{{$sub->id}}" {{$sub->id == $data->subcategory_id ? "selected":""}} >{{$sub->name}}</option>
													@endforeach                                                  
												@endif
											</select>
									</div>
								...
								<div class="{{ $data->measure == null ? 'showbox' : '' }}">
									<div class="row">
										...
										<div class="col-lg-12 {{ in_array($data->measure,explode(',', 'Gram,Kilogram,Litre,Pound')) ? 'hidden' : '' }}" id="measure">
											<input name="measure" type="text" id="measurement" class="form-control" placeholder="@lang('Enter Unit')" value="{{$data->measure}}">
										</div>
									</div>
								</div>
								...
								<div class="{{ $data->ship != null ? "":"showbox" }}">
									<div class="row">
										...
										<div class="col-lg-12">
											<input type="text" class="form-control" placeholder="@lang('Estimated Shipping Time')" name="ship" value="{{ $data->ship == null ? "" : $data->ship }}">
										</div>
									</div>
								</div>
...
<script type="text/javascript">
	...
	$(document).on("click", ".set-gallery", function () {
		...
		$.ajax({
			type: "GET",
			url: "{{ route('merchant-gallery-show') }}",
			data: {
				id: pid
			},
	...
	$(document).on('click', '.remove-img', function () {
		...
		$.ajax({
			type: "GET",
			url: "{{ route('merchant-gallery-delete') }}",
			data: {
				id: id
			}
		});
	...
	$(document).on('submit', '#form-gallery', function () {
		$.ajax({
			url: "{{ route('merchant-gallery-store') }}",
			method: "POST",
