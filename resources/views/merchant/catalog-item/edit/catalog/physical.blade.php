...
@section('content')
    <div class="gs-vendor-outlet">
        ...
        <form class="row gy-3 gy-lg-4 add-product-form" id="myForm"
            action="{{ route('merchant-catalog-item-catalog-update', $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
                            ...
                            <select class="form-control nice-select form__control subcategory" name="subcategory_id"
                                ...
                                @if ($data->subcategory_id == null)
                                    @foreach ($data->category->subs as $sub)
                                        <option data-href="{{ route('merchant-childcat-load', $sub->id) }}"
    ...
    <script>
        (function($) {
            ...
            $(document).ready(function() {
                ...
                let html =
                    `<img width="200" src="{{ empty($data->photo) ? asset('assets/images/noimage.png') : (filter_var($data->photo, FILTER_VALIDATE_URL) ? $data->photo : ($data->photo ? \Illuminate\Support\Facades\Storage::url($data->photo) : asset('assets/images/noimage.png'))) }}" alt="">`;
