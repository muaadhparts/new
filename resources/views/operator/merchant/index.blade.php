		...
		<div class="row">
			<div class="col-lg-12">
				...
				<ul class="links">
					...
					<li>
						<a href="{{ route('operator-merchant-index') }}">{{ __("Merchants List") }}</a>
					</li>
				</ul>
			</div>
		</div>
				...
				<div class="heading-area">
					...
					<div class="action-list">
						<select
							...
							<option data-val="1" value="{{route('operator-gs-status',['reg_merchant',1])}}" {{ $gs->
								reg_merchant == 1 ? 'selected' : '' }}>{{ __("Activated") }}</option>
							<option data-val="0" value="{{route('operator-gs-status',['reg_merchant',0])}}" {{ $gs->
...
<script type="text/javascript">
	(function($) {
		...
		var table = $('#muaadhtable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('operator-merchant-datatables') }}',
               columns: [
