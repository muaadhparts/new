            ...
            <div class="col-lg-12">
                <div class="catalogItem-description">
                    <div class="body-area">
                        <div class="row mb-4">
                            <div class="col-md-8">
                                <h5 class="mb-3">{{ __('Current Big Save CatalogItems') }}</h5>
                            </div>
                            <div class="col-md-4">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('Add CatalogItem to Big Save') }}</h5>
...
$(document).ready(function() {
    ...
    $('#catalogItemSearch').on('keyup', function() {
        ...
        searchTimeout = setTimeout(function() {
            ...
            $.get('{{ route("operator-fs-big-save-search") }}', { q: query }, function(data) {
    ...
    $(document).on('click', '.select-catalogItem', function() {
        ...
        $.get('{{ route("operator-fs-big-save-merchants") }}', { catalog_item_id: selectedCatalogItemId }, function(merchants) {
    ...
    $(document).on('click', '.add-item', function() {
        ...
        $.ajax({ url: '{{ route("operator-fs-big-save-toggle") }}', type: 'POST', data: { _token: '{{ csrf_token() }}', merchant_item_id: btn.data('id'), flag: 1 },
    ...
    $(document).on('click', '.remove-item', function() {
        ...
        btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i>');
        $.ajax({ url: '{{ route("operator-fs-big-save-toggle") }}', type: 'POST', data: { _token: '{{ csrf_token() }}', merchant_item_id: btn.data('id'), flag: 0 },
            success: function(r) { if (r.success) { row.fadeOut(300, function() { $(this).remove(); if ($('#productsTable tbody tr').length === 0) { $('#productsTable').html('<div class="alert alert-info">{{ __("No catalogItems configured.") }}</div>'); } }); } },
