                ...
                <div class="row g-3 align-items-end">
                    <div class="col-md-4">
                        ...
                        <input type="text" autocomplete="off" class="form-control discount_date"
                               ...
                               name="start_date" placeholder="{{ __('From Date') }}">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">{{ __('End Date') }}</label>
                        <input type="text" autocomplete="off" class="form-control discount_date"
                               value="{{ $end_date != '' ? $end_date->format('d-m-Y') : '' }}"
                               name="end_date" placeholder="{{ __('To Date') }}">
                    </div>
                    <div class="col-md-4">
    ...
    <div class="row mb-4">
        <div class="col-md-3">
            <div class="card bg-primary text-white">
                <div class="card-body text-center">
                    <h6>{{ __('Total Commission (Filtered)') }}</h6>
                    <h3>{{ $total }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-3">
            <div class="card bg-warning text-dark">
                <div class="card-body text-center">
                    <h6>{{ __('Avg Rate') }}</h6>
                    <h3>{{ $avg_commission_rate ?? 0 }}%</h3>
                </div>
            </div>
        </div>
    </div>

    {{-- Commission by Merchant Summary --}}
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{ $merchant['avg_commission_rate'] ?? 0 }}%</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#reset', function() {
        $('.discount_date').val('');
        location.href = '{{ route('operator-commission-income') }}';
    });
