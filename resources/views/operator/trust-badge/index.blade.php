...
@section('content')
					<input type="hidden" id="headerdata" value="{{ __("TRUST BADGES") }}">
					<div class="content-area">
						<div class="mr-breadcrumb">
							<div class="row">
								<div class="col-lg-12">
										<h4 class="heading">{{ __("Merchant Trust Badges") }}</h4>
										<ul class="links">
											<li>
												<a href="{{ route('operator.dashboard') }}">{{ __("Dashboard") }} </a>
											</li>
											<li><a href="javascript:;">{{ __('Merchant Trust Badges') }}</a></li>
											<li>
												<a href="{{ route('operator-trust-badge-index','all') }}">{{ __("All Trust Badges") }}</a>
											</li>
										</ul>
								</div>
							</div>
							...
							<div class="row">
								<div class="col-lg-12">
									<div class="heading-area">
										<h4 class="title">
											{{ __('Allow Trust Badge To Add CatalogItem') }} :
										</h4>
										<div class="action-list">
											<select class="process new-select {{ $gs->verify_item == 1 ? 'drop-success' : 'drop-danger' }}">
											  <option data-val="1" value="{{route('operator-gs-status',['verify_item',1])}}" {{ $gs->verify_item == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
											  <option data-val="0" value="{{route('operator-gs-status',['verify_item',0])}}" {{ $gs->verify_item == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
											</select>
										  </div>
									</div>
...
<div class="modal fade" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
  <div class="modal-dialog">
	...
	<div class="modal-header d-block text-center">
      ...
      <!-- Modal body -->
      <div class="modal-body">
            <p class="text-center">{{__("You are about to delete this Trust Badge.")}}</p>
...
(function($) {
		...
		var table = $('#muaadhtable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('operator-trust-badge-datatables','all') }}',
               columns: [
                        { data: 'name', name: 'name' },
