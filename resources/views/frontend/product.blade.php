                ...
                <div class="col-lg-6">
                    <div class="gs-product-details-gallery-wrapper">
                        <div class="product-main-slider">
                            <img
                                src="{{ filter_var($productt->photo, FILTER_VALIDATE_URL) ? $productt->photo : \Illuminate\Support\Facades\Storage::url($productt->photo)  }}"
                                alt="Thumb Image"
                        ...
                        <div class="product-nav-slider">
                            <img
                                src="{{ filter_var($productt->photo, FILTER_VALIDATE_URL) ? $productt->photo :  \Illuminate\Support\Facades\Storage::url($productt->thumbnail)  }}"
                                alt="Thumb Image"
                ...
                <div class="col-lg-6">
                    ...
                    @if(isset($merchant) && $merchant->qualityBrand && $merchant->qualityBrand->logo)
                        <div class="quality-brand-badge mb-3">
                            ...
                            <div class="quality-brand-info">
                                <span class="quality-brand-label">@lang('Brand qualities:')</span>
                                <span class="quality-brand-name">{{ app()->getLocale() == 'ar' && $merchant->qualityBrand->name_ar ? $merchant->qualityBrand->name_ar : $merchant->qualityBrand->name_en }}</span>
                            </div>
                        </div>
                    @endif
                    <form>
                        ...
                        <div class="product-header-info mb-3">
                            ...
                            @php
                                $vendorId = $vendorId ?? (isset($merchant) ? $merchant->user_id : 0);
                            ...
                            <div class="product-price-section d-flex align-items-center gap-3 mb-3">
                                <h3 class="product-current-price mb-0" id="sizeprice">{{ $currentPrice }}</h3>
                                @if($previousPrice != $currentPrice)
                                    <h5 class="product-old-price mb-0 text-muted"><del>{{ $previousPrice }}</del></h5>
                                @endif
                                @if(!is_null($offPercent) && $offPercent > 0)
                                    <span class="discount-badge">-{{ $offPercent }}%</span>
                                @endif
                            </div>

                            {{-- Rating --}}
                            <div class="product-rating d-flex align-items-center gap-2 mb-3">
                                <div class="rating-stars text-warning">
                                    <i class="fas fa-star"></i>
                                    <span class="rating-value fw-bold">{{ number_format($productt->ratings_avg_rating, 1) }}</span>
                                </div>
                                <span class="rating-count text-muted">
                                    ({{ App\Models\Rating::ratingCount($productt->id) }} @lang('Reviews'))
                                </span>
                            </div>
                        </div>
                            ...
                            <div class="details-list">
                                ...
                                @if ($productt->type == 'Physical')
                                    ...
                                    <div class="detail-item">
                                        ...
                                        @else
                                            <span class="badge bg-success">{{ ($gs->show_stock == 0 ? '' : $mpStock . ' ') }}{{ __('In Stock') }}</span>
                                        @endif
                                    </div>
                                @endif
                                ...
                                @if ($productt->sku != null)
                                    <div class="detail-item">
                                        <span class="detail-label">@lang('Product SKU :')</span>
                                        <a href="{{ route('search.result', $productt->sku) }}" target="_blank" class="text-primary fw-bold">
                                            {{ $productt->sku }}
                                        </a>
                                    </div>
                                @endif
                                ...
                                @if($productt->brand)
                                    <div class="detail-item">
                                        <span class="detail-label">@lang('Brand:')</span>
                                        <span class="detail-value">{{ Str::ucfirst($productt->brand->name) }}</span>
                                    </div>
                                @endif
                                ...
                                @if(isset($merchant) && $merchant->qualityBrand)
                                    <div class="detail-item">
                                        <span class="detail-label">@lang('Brand qualities:')</span>
                                        <span class="detail-value">{{ app()->getLocale() == 'ar' && $merchant->qualityBrand->name_ar ? $merchant->qualityBrand->name_ar : $merchant->qualityBrand->name_en }}</span>
                                    </div>
                                @endif
                                ...
                                @if ($productt->ship != null)
                                    <div class="detail-item">
                                        <span class="detail-label">@lang('Estimated Shipping Time :')</span>
                                        <span class="detail-value">{{ $productt->ship }}</span>
                                    </div>
                                @endif
                                ...
                                @if ($productt->type == 'License')
                                    @if ($productt->platform != null)
                                        <div class="detail-item">
                                            <span class="detail-label">@lang('Platform:')</span>
                                            <span class="detail-value">{{ $productt->platform }}</span>
                                        </div>
                                    @endif
                                    @if ($productt->region != null)
                                        <div class="detail-item">
                                            <span class="detail-label">@lang('Region:')</span>
                                            <span class="detail-value">{{ $productt->region }}</span>
                                        </div>
                                    @endif
                                    @if ($productt->licence_type != null)
                                        <div class="detail-item">
                                            <span class="detail-label">@lang('License Type:')</span>
                                            <span class="detail-value">{{ $productt->licence_type }}</span>
                                        </div>
                                    @endif
                                @endif
                            </div>
                        ...
                        <input type="hidden" id="product_price"
                               value="{{ round((($merchant && method_exists($merchant,'vendorSizePrice')) ? $merchant->vendorSizePrice() : $productt->vendorPrice()) * $curr->value, 2) }}">
                        <input type="hidden" id="vendor_id" value="{{ (int) $vendorId }}">
                        <input type="hidden" id="product_id" value="{{ $productt->id }}">
                        <input type="hidden" id="curr_pos" value="{{ $gs->currency_format }}">
                        <input type="hidden" id="curr_sign" value="{{ $curr->sign }}">

                        {{-- Compatibility & Alternative Products --}}
                        <div class="product-actions-top mb-3">
                            <div class="row g-2">
                                <div class="col-6">
                                    <livewire:compatibility :sku="$productt->sku" wire:key="'compat-'.$productt->sku" />
                                </div>
                                <div class="col-6">
                                    <livewire:alternativeproduct :sku="$productt->sku" wire:key="'alt-'.$productt->sku" />
                                </div>
                            </div>
                        </div>

                        {{-- Add to Cart & Buy Now Buttons --}}
                        <div class="product-purchase-actions mb-3">
                            <div class="row g-2">
                                <div class="col-6">
                                    <button type="button" class="btn btn-outline-primary w-100" id="addtodetailscart">
                                        <i class="fas fa-cart-plus me-1"></i>
                                        <span class="d-none d-md-inline">@lang('add to cart')</span>
                                        <span class="d-md-none">@lang('Cart')</span>
                                    </button>
                                </div>
                                <div class="col-6">
                                    <button type="button" class="btn btn-primary w-100" id="addtobycard">
                                        <i class="fas fa-shopping-bag me-1"></i>
                                        <span class="d-none d-md-inline">@lang('buy now')</span>
                                        <span class="d-md-none">@lang('Buy')</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="wish-compare-report-wrapper">
                            @if (Auth::check())
                                <a href="javascript:;" data-href="{{ route('user-wishlist-add', ['id' => $productt->id, 'user' => $vendorId]) }}"
                                   class="link wishlist">
                            ...
                            @if ($productt->type != 'Listing')
                                <a data-href="{{ route('product.compare.add', ['id' => $productt->id, 'user' => $vendorId]) }}"
                                   href="javascript:;" class="link compare_product">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                         viewBox="0 0 24 24" fill="none">
                                ...
                                @if (Auth::guard('web')->check())
                                    <a href="#" class="link report-item" href="javascript:;"
                                       data-bs-toggle="modal" data-bs-target="#report-modal">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                             viewBox="0 0 24 24" fill="none">
                                            <path d="M4 15C4 15 5 14 8 14C11 14 13 16 16 16C19 16 20 15 20 15V3C20 3 19 4 16 4C13 4 11 2 8 2C5 2 4 3 4 3L4 22"
                                                  stroke="#1F0300" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                ...
                                <a class="linkedin a2a_button_linkedin" href="#">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40"
                                        ...
                                        <g clip-path="url(#clip0_428_16041)">
                                            <path d="M20 40C31.0457 40 40 31.0457 40 20C40 8.95431 31.0457 0 20 0C8.95431 0 0 8.95431 0 20C0 31.0457 8.95431 40 20 40Z" fill="#0B69C7"/>
                                            <path d="M15.5257 12.7289C15.5262 13.2694 15.3664 13.7979 15.0664 14.2475C14.7665 14.6971 14.3399 15.0477 13.8406 15.2547C13.3414 15.4618 12.7919 15.5161 12.2618 15.4108C11.7317 15.3055 11.2447 15.0453 10.8626 14.6631C10.4804 14.2809 10.2202 13.794 10.1149 13.2638C10.0095 12.7337 10.0639 12.1843 10.2709 11.685C10.478 11.1858 10.8285 10.7592 11.2781 10.4593C11.7277 10.1593 12.2562 9.99948 12.7967 10C13.5203 10.0007 14.214 10.2884 14.7256 10.8001C15.2372 11.3117 15.525 12.0054 15.5257 12.7289Z" fill="white"/>
                                            <path d="M29.9336 23.5446V29.3789C29.9336 29.5436 29.8681 29.7015 29.7517 29.818C29.6352 29.9345 29.4772 29.9999 29.3125 29.9999H26.2283C26.0636 29.9999 25.9056 29.9345 25.7891 29.818C25.6727 29.7015 25.6072 29.5436 25.6072 29.3789V23.7262C25.6072 22.8815 25.852 20.042 23.402 20.042C21.502 20.042 21.1151 21.9946 21.0336 22.871V29.392C21.0301 29.5535 20.9639 29.7074 20.849 29.8208C20.734 29.9343 20.5793 29.9986 20.4178 29.9999H17.4336C17.3519 30.0003 17.271 29.9844 17.1955 29.9533C17.12 29.9223 17.0514 29.8765 16.9936 29.8188C16.9359 29.761 16.8902 29.6924 16.8591 29.6169C16.828 29.5414 16.8122 29.4605 16.8125 29.3789V17.221C16.8122 17.1392 16.828 17.0581 16.859 16.9825C16.8901 16.9068 16.9358 16.8381 16.9935 16.7801C17.0512 16.7222 17.1198 16.6762 17.1953 16.6448C17.2708 16.6134 17.3518 16.5973 17.4336 16.5973H20.4178C20.5006 16.5959 20.5828 16.611 20.6597 16.6417C20.7366 16.6724 20.8066 16.7181 20.8656 16.7762C20.9246 16.8342 20.9715 16.9035 21.0035 16.9798C21.0355 17.0562 21.052 17.1382 21.052 17.221V18.2736C21.7572 17.221 22.8046 16.3999 25.0336 16.3999C29.9625 16.3973 29.9336 21.0104 29.9336 23.5446Z" fill="white"/>
                                        </g>
                                        <defs><clipPath id="clip0_428_16041"><rect width="40" height="40" fill="white"/></clipPath></defs>
                                    </svg>
                                </a>
                        ...
                        <div class="seller-info-card card shadow-sm">
                            <div class="card-body">
                                ...
                                <div class="seller-details mb-3">
                                    <div class="seller-name mb-2">
                                        ...
                                        @if ($productt->user_id != 0)
                                            ...
                                            @endif
                                        @else
                                            <span class="fw-bold">{{ App\Models\Admin::find(1)->shop_name }}</span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="seller-actions d-flex flex-wrap gap-2">
                                    ...
                                    @if ($gs->is_contact_seller == 1)
                                        @if (Auth::check())
                                            ...
                                            @else
                                                <a class="btn btn-outline-secondary btn-sm" href="javascript:;" data-bs-toggle="modal" data-bs-target="#sendMessage">
                                                    <i class="fas fa-comment me-1"></i>
                                                    @lang('contact seller')
                                                </a>
                                            @endif
                                        @else
                                            <a class="btn btn-outline-secondary btn-sm" href="{{ route('user.login') }}">
                                                <i class="fas fa-comment me-1"></i>
                                                {{ __('Contact Seller') }}
                                            </a>
                                        @endif
                                    ...
                                    @if ($productt->user_id != 0)
                                        @if (Auth::check())
                                            ...
                                            @endif
                                        @else
                                            <a class="btn btn-outline-success btn-sm" href="{{ route('user.login') }}">
                                                <i class="fas fa-plus me-1"></i>
                                                {{ __('Add To Favorite Seller') }}
                                            </a>
                                        @endif
        ...
        <div class="container">
            <ul class="nav nav-tabs flex-wrap" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link small" id="description-tab" data-bs-toggle="tab"
                ...
                {{-- Alternatives Tab - Active by Default --}}
                <li class="nav-item" role="presentation">
                    <button class="nav-link active small" id="alternatives-tab" data-bs-toggle="tab" data-bs-target="#alternatives-tab-pane"
                            type="button" role="tab" aria-controls="alternatives-tab-pane" aria-selected="true">
                        <i class="fas fa-exchange-alt d-md-none"></i>
                        <span class="d-none d-md-inline">{{ __('Alternatives') }}</span>
                        <span class="d-md-none">{{ __('Alt') }}</span>
                    </button>
                </li>
            ...
            <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade" id="reviews-tab-pane" role="tabpanel" aria-labelledby="reviews-tab" tabindex="0">
                    <div class="row review-tab-content-wrapper">
                        <div class="col-xxl-8">
                            <div id="comments">
                                <h5 class="MUAADH-Reviews-titleDDD my-3">@lang('Ratings & Reviews')</h5>
                                <ul class="all-comments">
                                        ...
                                        <li>
                                            <div class="single-comment">
                                                <div class="left-area">
                                                    ...
                                                    <p class="name text-lg">{{ $review->user->name }}</p>
                                                    <div class="reating-area">
                                                        <div class="stars"><span id="star-rating">{{ $review->rating }}</span> <i class="fas fa-star"></i></div>
                                    ...
                                    @empty
                                        <li>
                                            <div class="single-comment">
                                                <div class="left-area">
                                                    <p class="name text-lg">@lang('No Review Found')</p>
                                                </div>
                                            </div>
                                        </li>
                                    @endforelse
                            ...
                            @else
                                <h5 class="text-center">
                                    <a href="{{ route('user.login') }}" class="btn login-btn mr-1">{{ __('Login') }}</a>
                                    {{ __('To Review') }}
                                </h5>
                            @endif
                        </div>
                ...
                {{-- Alternatives Tab content start - Active by Default --}}
                <div class="tab-pane fade show active" id="alternatives-tab-pane" role="tabpanel" aria-labelledby="alternatives-tab" tabindex="0">
                    <div class="alternatives-content-wrapper">
                        <div class="mb-4">
                            <h5 class="mb-3">
                            ...
                            <p class="text-muted">
                                {{ __('Browse alternative products that are compatible with your vehicle') }}
                            </p>
                        </div>

                        <div class="alternatives-products-grid">
                            @livewire('alternativeproduct', ['sku' => $productt->sku, 'displayMode' => 'inline'], key('alternatives-tab-'.$productt->sku))
                        </div>
                    </div>
                </div>
                {{-- Alternatives Tab content end --}}

                @if ($productt->whole_sell_qty != null && $productt->whole_sell_qty != '')
                    <!-- Wholesell Tab content start  -->
                    <div class="tab-pane fade" id="whole-sell-tab-pane" role="tabpanel" aria-labelledby="whole-sell-tab" tabindex="0">
                                            ...
                                            <ul class="price-summary-list">
                                                ...
                                                @foreach ($productt->whole_sell_qty as $key => $data1)
                                                    <li class="selling-price">
                                                        ...
                                                        <span><span class="MUAADH-Price-amount amount">{{ $productt->whole_sell_discount[$key] }}% @lang('Off')</span></span>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </div>
            ...
            <form action="{{ route('product.report') }}" method="POST"
                ...
                <div class="modal-content assign-rider-modal-content form-group">
                    ...
                    <div class="input-label-wrapper w-100">
                        ...
                        <textarea name="note" class="form-control border p-3" placeholder="{{ __('Enter Report Note') }}" required=""></textarea>
                    </div>
            ...
            <div class="modal-content assign-rider-modal-content form-group">
                ...
                <div class="input-label-wrapper w-100">
                    <div class="dropdown-container">
                        <input type="text" class="form-control form__control border px-3 mb-4" name="subject" placeholder="@lang('Subject')" required="">
                        <textarea class="form-control form__control border px-3 mb-4" name="message" placeholder="{{ __('Your Message') }}" required=""></textarea>
                    </div>
                </div>
...
@section('script')
            ...
            $(document).on("submit", "#emailreply", function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data == 0) toastr.error("Email Not Found");
                        else toastr.success("Message Sent");
    ...
    .product-nav-slider img:hover,
    .product-nav-slider img.active {
    ...
    .single-product-details-content-wrapper h2,
    .single-product-details-content-wrapper h3 {
    ...
    /* ========================================
       NEW ENHANCED PRODUCT PAGE STYLES
       ======================================== */
    ...
    .product-purchase-actions .btn:hover {
        transform: translateY(-2px);
    ...
    .alternatives-content-wrapper h5 {
        color: var(--dark-color);
        font-weight: 700;
        font-size: 1.5rem;
    }

    .alternatives-content-wrapper .text-muted {
        font-size: 1rem;
        line-height: 1.6;
    }

    .alternatives-products-grid {
        background: #fff;
        padding: 1.5rem;
        border-radius: var(--border-radius);
        box-shadow: var(--box-shadow-sm);
    ...
    .tab-product-des-wrapper .nav-link i {
        margin-right: 0.5rem;
    }

    /* Responsive for Alternatives Tab */
    @media (max-width: 991px) {
        .alternatives-content-wrapper {
            padding: 1.5rem;
        }
