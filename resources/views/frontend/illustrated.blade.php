...
{{--                                        @dd($category)--}}
                                    ...
                                    @php
                                    // dd($category->images);
                                        // $image = str_replace('["', '', $category->images);
                                        // $image = str_replace('"]', '', $image);
                                        // $image = str_replace('\\', '', $image);
                                         app()->getLocale();
//
                                            $image ='https://blr1.digitaloceanspaces.com/cdn.part.sa/LargePNG/J10100ST___L.png';

                                        // $newImage = Image::make($image);
                                        // $width = $newImage->width();
                                        // $height = $newImage->height();
                                        // $Width = Image::load($image)->getWidth();
                                        // $Height = Image::load($image)->getHeight();
//                                        try {
                                            //code...
//                                            $partCallouts = collect($category->illustrationWithCallouts)['partCallouts'];
//                                            $partCallouts = collect($category->illustrationWithCallouts)['partCallouts'];
//                                            dd($partCallouts ,$category->illustrationWithCallouts);
//                                            $parts = collect($category->catalogWithParts)['parts'];
                                            //  = collect($category->illustrationWithCallouts)['partCallouts'];
//                                        } catch (\Throwable $th) {
//                                            $partCallouts = [];
//                                             dd($partCallouts);
//                                            Log::error('no partCallouts' . json_encode($category));
//                                            //throw $th;
//                                        }
                                        // dd($partCallouts);
                                        // //
                                        // dump($image,$partCallouts);
                                    @endphp
