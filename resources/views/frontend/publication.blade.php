        ...
        <div class="container">
            <div class="row flex-column-reverse flex-lg-row">
                    ...
                    <div class="gs-blog-sidebar-wrapper">
                            ...
                            <ul class="cat-wrapper">
                                @foreach ($bcats as $cat)
                                    <li><a class="{{ isset($bcat) ? ($bcat->id == $cat->id ? 'active' : '') : '' }}"
                                            href="{{ route('front.publicationscategory', $cat->slug) }}">{{ $cat->name }}
                                            ({{ $cat->publications_count }})
                                        </a></li>
                                @endforeach
                            </ul>
                        </div>
                        ...
                        <div class="single-blog-widget wow-replaced" data-wow-delay=".1s">
                            ...
                            <div class="gs-sm-recent-post-wrapper">
                                ...
                                @foreach (App\Models\Publication::latest()->limit(4)->get() as $recent)
                                    <a href="{{ route('front.publicationshow', $recent->slug) }}" class="recent-post d-flex">
                                    ...
                                    @if (!empty($tag))
                                        <li>
                                            <a class="{{ isset($slug) ? ($slug == $tag ? 'active' : '') : '' }}"
                                                href="{{ route('front.publicationstags', $tag) }}">
                                                {{ $tag }}
                                            </a>
                                        </li>
                                    @endif
                ...
                <div class="col-12 col-lg-8 gs-main-blog-wrapper wow-replaced" data-wow-delay=".1s">
                    @forelse ($publications as $publication)
                        <div class="gs-main-single-blog wow-replaced " data-wow-delay=".1s">
                            <div class="left-side-content">
                                <img class="blog-img"
                                    src="{{ $publication->photo ? asset('assets/images/publications/' . $publication->photo) : asset('assets/images/noimage.png') }}"
                                    alt="blog img">
                            </div>
                            <div class="right-side-content">
                                ...
                                <p class="des">
                                    {{ mb_strlen(strip_tags($publication->details), 'UTF-8') > 150
                                        ? mb_substr(strip_tags($publication->details), 0, 150, 'UTF-8') . '...'
                                        : strip_tags($publication->details) }}
                                </p>
                                ...
                                <div class="date-wrapper">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        ...
                                        <path
                                            ...
                                            stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M15.6937 13.7H15.7027" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M15.6937 16.7H15.7027" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M11.9945 13.7H12.0035" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M11.9945 16.7H12.0035" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M8.29529 13.7H8.30427" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M8.29529 16.7H8.30427" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>

                                    <span class="date-text">{{ date('M d - Y', strtotime($publication->created_at)) }}</span>
                                </div>

                                <a class="template-btn outlinee-btn"
                                    href="{{ route('front.publicationshow', $publication->slug) }}">@lang('read more')</a>
                            </div>
                        </div>
                    @empty
                    ...
                    @endforelse
                    {{ $publications->links('includes.frontend.pagination') }}
                </div>
            </div>
        </div>
    </div>
