...
@section('content')
            ...
            <form class="address-wrapper" action="{{ route('front.checkout.step2.submit') }}" method="POST">
                ...
                <div class="row gy-4">
                    <div class="col-lg-7 col-xl-8 wow-replaced" data-wow-delay=".2s">
                        {{-- ======= ترجمة + فول-باك لأسماء الدولة/الولاية/المدينة ======= --}}
                        @php
                            if (!function_exists('tr_or')) {
                                function tr_or($group, $value) {
                                    if ($value === null || $value === '') return '';
                                    $key = $group . '.' . $value;
                                    $t = __($key);
                                    return $t === $key ? $value : $t; // fallback للنص الأصلي
                                }
                            }

                            // قيم الفوترة: إن كانت IDs نستخرج الاسم من DB، وإن كانت نصوص نستخدمها كما هي
                            $billingStateName = is_numeric($step1->customer_state ?? null)
                                ? optional(\App\Models\State::find($step1->customer_state))->state
                                : ($step1->customer_state ?? '');

                            $billingCityName = is_numeric($step1->customer_city ?? null)
                                ? optional(\App\Models\City::find($step1->customer_city))->city_name
                                : ($step1->customer_city ?? '');

                            // قيم الشحن
                            $shippingStateName = is_numeric($step1->shipping_state ?? null)
                                ? optional(\App\Models\State::find($step1->shipping_state))->state
                                : ($step1->shipping_state ?? '');

                            $shippingCityName = is_numeric($step1->shipping_city ?? null)
                                ? optional(\App\Models\City::find($step1->shipping_city))->city_name
                                : ($step1->shipping_city ?? '');
                        @endphp
                                ...
                                <ul>
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            ...
                                            <path d="M11.9999 15C8.82977 15 6.01065 16.5306 4.21585 18.906C3.82956 19.4172 3.63641 19.6728 3.64273 20.0183C3.64761 20.2852 3.81521 20.6219 4.02522 20.7867C4.29704 21 4.67372 21 5.42708 21H18.5726C19.326 21 19.7027 21 19.9745 20.7867C20.1845 20.6219 20.3521 20.2852 20.357 20.0183C20.3633 19.6728 20.1701 19.4172 19.7839 18.906C17.9891 16.5306 15.1699 15 11.9999 15Z" stroke="#1F0300" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                            <path d="M11.9999 12C14.4851 12 16.4999 9.98528 16.4999 7.5C16.4999 5.01472 14.4851 3 11.9999 3C9.51457 3 7.49985 5.01472 7.49985 7.5C7.49985 9.98528 9.51457 12 11.9999 12Z" stroke="#1F0300" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    ...
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none">
                                            <path d="M12 12.5C13.6569 12.5 15 11.1569 15 9.5C15 7.84315 13.6569 6.5 12 6.5C10.3431 6.5 9 7.84315 9 9.5C9 11.1569 10.3431 12.5 12 12.5Z" stroke="#1F0300" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                            <path d="M12 22C14 18 20 15.4183 20 10C20 5.58172 16.4183 2 12 2C7.58172 2 4 5.58172 4 10C4 15.4183 10 18 12 22Z" stroke="#1F0300" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    ...
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none">
                                            <path d="M2 7L10.1649 12.7154C10.8261 13.1783 11.1567 13.4097 11.5163 13.4993C11.8339 13.5785 12.1661 13.5785 12.4837 13.4993C12.8433 13.4097 13.1739 13.1783 13.8351 12.7154L22 7M6.8 20H17.2C18.8802 20 19.7202 20 20.362 19.673C20.9265 19.3854 21.3854 18.9265 21.673 18.362C22 17.7202 22 16.8802 22 15.2V8.8C22 7.11984 22 6.27976 21.673 5.63803C21.3854 5.07354 20.9265 4.6146 20.362 4.32698C19.7202 4 18.8802 4 17.2 4H6.8C5.11984 4 4.27976 4 3.63803 4.32698C3.07354 4.6146 2.6146 5.07354 2.32698 5.63803C2 6.27976 2 7.11984 2 8.8V15.2C2 16.8802 2 17.7202 2.32698 18.362C2.6146 18.9265 3.07354 19.3854 3.63803 19.673C4.27976 20 5.11984 20 6.8 20Z" stroke="#1F0300" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                        <span class="title">{{ $step1->customer_email }}</span>
                                    </li>

                                    {{-- أسماء مترجمة مع فول-باك --}}
                            ...
                            @if (isset($step1->is_shipping) && $step1->is_shipping)
                                <div class="single-addres">
                                    ...
                                    <ul>
                                        ...
                                        @endif
                                        @if ($step1->shipping_country)
                                            <li><span class="title">{{ tr_or('countries', $step1->shipping_country) }}</span></li>
                                        @endif
                                        @if ($shippingStateName)
                                            <li><span class="title">{{ tr_or('states', $shippingStateName) }}</span></li>
                                        @endif
                                        @if ($shippingCityName)
                                            <li><span class="title">{{ tr_or('cities', $shippingCityName) }}</span></li>
                                        @endif
                                    </ul>
                                </div>
                            @endif
                        ...
                        @foreach ($resultArray as $vendor_id => $array_product)
                            @php
                                ...
                                // هل لدى هذا البائع منتجات فيزيكال؟
                                $vendor_has_physical = false;
                                foreach ($array_product as $p) { if (($p['dp'] ?? 1) == 0) { $vendor_has_physical = true; break; } }
                            @endphp
                    ...
                    <div class="col-lg-5 col-xl-4 wow-replaced" data-wow-delay=".2s">
                        <div class="summary-box">
                            ...
                            @if ($digital == 0)
                                <!-- shipping methods (single shipping) -->
                                @if ($gs->multiple_shipping == 0)
    ...
    @php
        ...
        // Boolean flag فقط: 1 إذا وُجدت ولاية، غير ذلك 0
        $isState = isset($step1->customer_state) ? 1 : 0;
    @endphp
    <input type="hidden" id="select_country" name="country_id" value="{{ $country?->id }}">
    <input type="hidden" id="state_id" name="state_id" value="{{ $step1->customer_state ?? 0 }}">
    <input type="hidden" id="is_state" name="is_state" value="{{ $isState }}">
    <input type="hidden" id="state_url" name="state_url" value="{{ route('country.wise.state', $country?->id ?? 0) }}">
@endsection
