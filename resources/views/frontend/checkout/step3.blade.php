...
<div class="gs-checkout-wrapper">
    <div class="container">
        ...
        <form class="address-wrapper checkoutform" method="POST">
            ...
            {{-- Use totalPrice from controller (includes products + shipping from step2) --}}
...
<script type="text/javascript">
    ...
    // المهم: استدع الضريبة بعد اكتمال التحميل لضمان امتلاء الحقول
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 0) {
                    ...
                    if (data[4] != 0) { $('.dpercent').html('(' + data[4] + ')'); } else { $('.dpercent').html(''); }

                    var ttotal = data[6] + parseFloat(mship) + parseFloat(mpack);
                    ttotal = parseFloat(ttotal);
                    if (ttotal % 1 != 0) { ttotal = ttotal.toFixed(2); }

                    if (pos == 0) { $('.total-amount').html('{{ $curr->sign }}' + ttotal) }
                    else          { $('.total-amount').html(ttotal + '{{ $curr->sign }}') }
    ...
    $('.payment').on('click', function () {
        if ($(this).data('val') == 'paystack') {
            ...
            $('.checkoutform').attr('id', 'mercadopago'); checkONE = 1;
        } else {
