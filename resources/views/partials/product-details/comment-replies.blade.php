<div class="row">
  <div class="col-8">
    <div id="comments">
      ...
      @if (Auth::check())
        ...
        <form id="comment-form" action="{{ route('product.comment') }}" method="POST">
          ...
          <div class="row">
          <div class="col-lg-12">
            <textarea class="form-control border" placeholder="{{ __('Write Your Comments Here...') }}" name="text"
            required></textarea>
          </div>
          </div>
          <div class="row mt-3">
    ...
    @endphp
      ...
      <div class="left-area">
      <img
      src="{{$comment->user->photo != null ? asset('assets/images/users/' . $comment->user->photo) : asset('assets/images/' . $gs->user_image)}}"
      alt="">
    ...
    @endif
      ...
      <a href="javascript:;" data-href="{{ route('product.comment.delete', $comment->id) }}"
      class="comment-link comment-delete mr-2"><i class="fas fa-trash"></i>{{ __('Delete') }}</a>
    @endif
      </div>
      </div>
      </div>
      ...
      <div class="left-area">
      <img
      src="{{ $reply->user->photo != null ? asset('assets/images/users/' . $reply->user->photo) : asset('assets/images/' . $gs->user_image) }}"
      alt="">
      ...
      <a href="javascript:;" class="comment-link edit mr-2"><i class="fas fa-edit "></i>{{ __('Edit') }}</a>
      <a href="javascript:;" data-href="{{ route('product.reply.delete', $reply->id) }}"
      class="comment-link reply-delete mr-2"><i class="fas fa-trash"></i>{{ __('Delete') }}</a>
    @endif
      </div>
      </div>
      </div>
    ...
    @else
      ...
      <div class="col-lg-12">
        <br>
        <h5 class="text-center"><a href="{{ route('user.login') }}" class="btn login-btn">{{ __('Login') }}</a>
        {{ __('To Comment') }} </h5>
        <br>
      </div>
      </div>
