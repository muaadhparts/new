<div class="product-info text-center">
  ...
  <div class="product-color">
    ...
    <ul class="color-list">
      ...
      <li
        class="{{ $loop->first ? 'active' : '' }} {{ $productt->IsSizeColor($vendorSizes[$key] ?? '') ? str_replace(' ','',($vendorSizes[$key] ?? '')) : ''  }} {{ ($vendorSizes[$key] ?? '') == ($vendorSizes[0] ?? '') ? 'show-colors' : '' }}">
        <span class="box" data-color="{{ $vendorColors[$key] ?? '' }}"
          style="background-color: {{ $vendorColors[$key] ?? '' }}">
