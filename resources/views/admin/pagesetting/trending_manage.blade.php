        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <ul class="links">
                    ...
                    <li><a href="{{ route('admin-ps-trending') }}">{{ __('Trending') }}</a></li>
                </ul>
            </div>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('#catalogItemSearch').on('keyup', function() {
        ...
        searchTimeout = setTimeout(function() {
            ...
            $.get('{{ route("admin-ps-trending-search") }}', { q: query }, function(data) {
    ...
    $(document).on('click', '.select-catalogItem', function() {
        ...
        $.get('{{ route("admin-ps-trending-merchants") }}', { catalog_item_id: selectedCatalogItemId }, function(merchants) {
    ...
    $(document).on('click', '.add-item', function() {
        ...
        $.ajax({ url: '{{ route("admin-ps-trending-toggle") }}', type: 'POST', data: { _token: '{{ csrf_token() }}', merchant_item_id: btn.data('id'), flag: 1 },
    ...
    $(document).on('click', '.remove-item', function() {
        ...
        btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i>');
        $.ajax({ url: '{{ route("admin-ps-trending-toggle") }}', type: 'POST', data: { _token: '{{ csrf_token() }}', merchant_item_id: btn.data('id'), flag: 0 },
