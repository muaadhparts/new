                  ...
                  <strong for="" class="text-capitalize">{{ str_replace("_", " ", $attrKey) }} :</strong>
                    ...
                    @foreach ($attrVal['values'] as $optionKey => $optionVal)
                      <div class="custom-control custom-radio">
                        ...
                        <input type="radio" id="{{$attrKey}}{{ $optionKey }}" name="{{ $attrKey }}" class="custom-control-input catalogItem-attr"  data-key="{{ $attrKey }}" data-price = "{{ $attrVal['prices'][$optionKey] * $curr->value }}" value="{{ $optionVal }}" {{ $loop->first ? 'checked' : '' }}>
