...
<option value="" disabled selected >{{ __('Select Country') }}</option>
	...
	<option value="{{ $data->country_name }}" data="{{$data->id}}" rel="{{$data->states->count() > 0 ? 1 : 0}}" rel1="{{ Auth::check() ? 1 : 0 }}" rel5="{{ Auth::check() ? 1 : 0 }}" data-href="{{route('country.wise.state',$data->id)}}">{{ $data->country_name }}</option>
@endforeach
