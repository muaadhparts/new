        ...
        <div class="container">
            <div class="d-flex">
                ...
                <div class="gs-user-conversation-content-wrapper gs-dashboard-outlet">
                                    ...
                                    <div>
                                        <div class="message-sender-box">
                                            @if ($message->chatThread->sent->is_provider == 1)
                                                <img src="{{ $message->chatThread->sent->photo != null ? $message->chatThread->sent->photo : asset('assets/images/noimage.png') }}"
                                                    alt="" class="avater">
                                            @else
                                                <img src="{{ $message->chatThread->sent->photo != null ? asset('assets/images/users/' . $message->chatThread->sent->photo) : asset('assets/images/noimage.png') }}"
                                                    alt="" class="avater">
                                            @endif
                                            <p class="message-sender">{{ $message->chatThread->sent->name }}</p>
                                        </div>
                                    </div>
                            ...
                            @else
                                ...
                                <div class="conversation-box conversation-right wow-replaced" data-wow-delay=".1s">
                                    ...
                                    <div>
                                        <div class="message-sender-box">
                                            @if ($message->chatThread->sent->is_provider == 1)
                                                <img src="{{ $message->chatThread->recieved->photo != null ? $message->chatThread->recieved->photo : asset('assets/images/noimage.png') }}"
                                                    alt="" class="avater">
                                            @else
                                                <img src="{{ $message->chatThread->recieved->photo != null ? asset('assets/images/users/' . $message->chatThread->recieved->photo) : asset('assets/images/noimage.png') }}"
                                                    alt="" class="avater">
                                            @endif
                                            <p class="message-sender">{{ $message->chatThread->recieved->name }}</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
                    ...
                    <!-- send message form  -->
                    <form action="{{ route('user-chat-post') }}" method="POST" class="send-message-form wow-replaced"
                        data-wow-delay=".1s">
                        @csrf

                        <input type="hidden" name="chat_thread_id" value="{{ $conv->id }}">
